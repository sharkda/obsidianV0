...
| 2    | #2  | **✓ Done 2026-06-25** (working tree, not committed). Stop GPS on `scenePhase` background | 2      | 6          | +4          | One handler at app root; `stopUpdateLocation()` on `.background`, restart on `.active`.                          |
| 3    | #3  | **✓ Done 2026-06-25** (working tree, not committed). Cancel repeating timers on background | 3      | 5          | +2          | Same handler as #2. Narrowed to the active protos' daily/minutely timers (not CyclopsObs/SourceBase — view-local). |
...
- ~~**#2 + #3 together** via one `scenePhase` handler~~ ✓ done 2026-06-25 (working tree, not committed). ~36 lines across 4 files (`Municipal+Lifecycle.swift` new, `hootowlApp.swift` / `Mu1Base+Ext.swift` / `Mu1Proto.swift` modified), no architectural change, easily reversible. Scope narrowed to GPS + active protos.
- **#5** (newly eligible post-cleanup) — Dedupe the redundant `self.locationMan = CLLocationManager()` in `Municipal.swift:122` + `:160`. ~1 line. Code-health, not battery, but trivial. **Only remaining pre-release fruit.**

All four high-value picks (#1, #6, #9, #2+#3) are now implemented; only #5 (trivial code-health) is left. Skip the rest until post-launch measurement.
