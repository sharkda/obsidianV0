...
**`main` = `origin/main` = `5d4d794`. Working tree clean.**
🌿 **On branch `destination-mode`** — four commits, **not merged**, awaiting Jim's test. Outside Taipei the app no longer refuses: it asks *"where are you heading?"*, and choosing a city makes everything work from there. **Reversible by construction** — with no destination set, `effectiveCentre` *is* the GPS fix, so the old path is the default path. Each commit reverts independently; `git checkout main` drops all of it. → **[[destination-mode]]**
